/* 한국표준질병·사인분류(KCD) 3자리 코드 — <b>한 벌</b>.

   ★ 이 표의 이름은 <b>사람이 타이핑하지 않았습니다.</b>
     scripts/kcd-fetch.js 가 질병분류기호(kcdcode.kr) 색인에서 받아 온 그대로입니다.
     두 번이나 외워 적었다가 틀렸기 때문입니다 —
       · 뇌·심장 범위표에서 아홉 자리 (거미막밑출혈→거미막하출혈, 폐색·협착→폐쇄 및 협착 …)
       · 새 질병 18종을 넣으며 열셋 중 여섯 (D01·D12·D33·H93·R42·N13·M05)
     이제 이름이 필요하면 <b>여기서 꺼내 씁니다.</b>

   ★ 공식 조회처인 KOICD 는 로그인을 요구해 들어가지 못했습니다.
     그래서 <b>어디서 받았는지를 화면에도 적습니다.</b> 원문 대조가 필요하면
     KOICD 화면과 한 줄씩 맞춰 봐야 합니다.

   ★ <b>이 표는 담보를 정하지 않습니다.</b> 코드가 어느 담보에 걸리는지는
     그 상품 <b>약관의 분류표</b>가 정하고, 회사·개정회차마다 다릅니다.
     여기서는 「어느 갈래를 보십시오」 까지만 말합니다.

   쓰는 법
     DZ_KCD.rows            [코드, 이름] 2,080벌
     DZ_KCD.kind(code)      '질병' | '외인' | '증상' | '보건' | '한의'
     DZ_KCD.area(code)      '암' | '뇌·심장' | '수술·입원' | '치매·간병' | null
     DZ_KCD.dz(code)        가장 가까운 상담자료의 질병 id | null
     DZ_KCD.find(q, max)    코드·이름으로 찾기                                     */
(function () {
  'use strict';

  var SRC = {
    n: '한국표준질병·사인분류(KCD) — 질병분류기호 색인',
    u: 'https://kcdcode.kr/mobile/index/1',
    got: '2026-08-23',
    note: '공식 조회처 KOICD 는 로그인을 요구해 들어가지 못했습니다.'
  };

  /* [코드, 이름] — scripts/kcd-fetch.js 가 받아 온 그대로 */
  var ROWS = [
  ['A00','콜레라'],
  ['A01','장티푸스 및 파라티푸스'],
  ['A02','기타 살모넬라감염'],
  ...
  ['A26','유사단독'],
  ['A27','렙토스피라병'],
  ['A28','달리 분류되지 않은 기타 동물매개의 세균성 질환'],
  ['A30','나병[한센병]'],
  ['A31','기타 형태의 마이코박테리아에 의한 감염'],
  ['A32','리스테리아증'],
  ['A33','신생아파상풍'],
  ['A34','산과적 파상풍'],
  ['A35','기타 파상풍'],
  ...
  ['A63','달리 분류되지 않은 기타 주로 성행위로 전파되는 질환'],
  ['A64','상세불명의 성매개질환'],
  ['A65','비성병성 매독'],
  ['A66','요스'],
  ['A67','핀타'],
  ...
  ['A83','모기매개바이러스뇌염'],
  ['A84','진드기매개바이러스뇌염'],
  ['A85','달리 분류되지 않은 기타 바이러스뇌염'],
  ['A86','상세불명의 바이러스뇌염'],
  ['A87','바이러스수막염'],
  ['A88','달리 분류되지 않은 중추신경계통의 기타 바이러스감염'],
  ['A89','중추신경계통의 상세불명의 바이러스감염'],
  ['A92','기타 모기매개바이러스열'],
  ['A93','달리 분류되지 않은 기타 절지동물매개바이러스열'],
  ['A94','상세불명의 절지동물매개 바이러스열'],
  ['A95','황열'],
  ['A96','아레나바이러스출혈열'],
  ['A97','뎅기'],
  ['A98','달리 분류되지 않은 기타 바이러스출혈열'],
  ['A99','상세불명의 바이러스출혈열'],
  ...
  ['B06','풍진'],
  ['B07','바이러스사마귀'],
  ['B08','달리 분류되지 않은 피부 및 점막병변이 특징인 기타 바이러스감염'],
  ['B09','피부 및 점막병변이 특징인 상세불명의 바이러스감염'],
  ['B15','급성 A형간염'],
  ['B16','급성 B형간염'],
  ['B17','기타 급성 바이러스간염'],
  ['B18','만성 바이러스간염'],
  ['B19','상세불명의 바이러스간염'],
  ['B20','감염성 및 기생충성 질환을 유발한 사람면역결핍바이러스병'],
  ['B21','악성 신생물을 유발한 사람면역결핍바이러스병'],
  ['B22','기타 명시된 질환을 유발한 사람면역결핍바이러스병'],
  ['B23','기타 병태를 유발한 사람면역결핍바이러스병'],
  ['B24','상세불명의 사람면역결핍바이러스병'],
  ['B25','거대세포바이러스병'],
  ['B26','볼거리'],
  ['B27','감염성 단핵구증'],
  ['B30','바이러스결막염'],
  ['B33','달리 분류되지 않은 기타 바이러스질환'],
  ['B34','상세불명 부위의 바이러스감염'],
  ['B35','백선증'],
  ['B36','기타 표재성 진균증'],
  ['B37','칸디다증'],
  ...
  ['B46','접합진균증'],
  ['B47','진균종'],
  ['B48','달리 분류되지 않은 기타 진균증'],
  ['B49','상세불명의 진균증'],
  ['B50','열대열원충말라리아'],
  ['B51','삼일열원충말라리아'],
  ['B52','사일열원충말라리아'],
  ['B53','기타 기생충학적으로 확인된 말라리아'],
  ['B54','상세불명의 말라리아'],
  ['B55','리슈만편모충증'],
  ['B56','아프리카파동편모충증'],
  ['B57','샤가스병'],
  ['B58','톡소포자충증'],
  ['B60','달리 분류되지 않은 기타 원충질환'],
  ['B64','상세불명의 원충병'],
  ['B65','주혈흡충증'],
  ['B66','기타 흡충감염'],
  ['B67','포충증'],
  ...
  ['B91','회색질척수염의 후유증'],
  ['B92','나병의 후유증'],
  ['B94','기타 및 상세불명의 감염성 및 기생충성 질환의 후유증'],
  ['B95','다른 장에서 분류된 질환의 원인으로서의 연쇄알균 및 포도알균'],
  ['B96','다른 장에서 분류된 질환의 원인으로서의 기타 명시된 세균감염체'],
  ['B97','다른 장에서 분류된 질환의 원인으로서의 바이러스감염체'],
  ['B98','다른 장에서 분류된 질환의 원인으로서의 기타 명시된 감염체'],
  ['B99','기타 및 상세불명의 감염성 질환'],
  ['C00','입술의 악성 신생물'],
  ['C01','혀뿌리의 악성 신생물'],
  ...
  ['D76','림프세망 및 세망조직구 조직의 참여를 동반한 기타 명시된 질환'],
  ['D77','달리 분류된 질환에서 혈액 및 조혈기관의 기타 장애'],
  ['D80','항체결손이 현저한 면역결핍'],
  ['D81','복합면역결핍'],
  ['D82','기타 주요 결손과 관련된 면역결핍'],
  ['D83','공통 가변성 면역결핍'],
  ['D84','기타 면역결핍'],
  ['D86','사르코이드증'],
  ['D89','달리 분류되지 않은 면역메커니즘을 침범한 기타 장애'],
  ['E00','선천성 요오드결핍증후군'],
  ...
  ['E28','난소의 기능이상'],
  ['E29','고환의 기능이상'],
  ['E30','달리 분류되지 않은 사춘기의 장애'],
  ['E31','다선성 기능이상'],
  ['E32','흉선의 질환'],
  ['E34','기타 내분비장애'],
  ['E35','달리 분류된 질환에서의 내분비선장애'],
  ['E40','콰시오르코르'],
  ['E41','영양성 소모증'],
  ['E42','소모성 콰시오르코르'],
  ['E43','상세불명의 중증 단백질-에너지영양실조'],
  ['E44','중등도 및 경도의 단백질-에너지영양실조'],
  ['E45','단백질-에너지영양실조로 인한 발육지연'],
  ...
  ['E88','기타 대사장애'],
  ['E89','달리 분류되지 않은 처치후 내분비 및 대사 장애'],
  ['E90','달리 분류된 질환에서의 영양 및 대사 장애'],
  ...
  ['F10','알코올사용에 의한 정신 및 행동 장애'],
  ['F11','아편유사제 사용에 의한 정신 및 행동 장애'],
  ['F12','카나비노이드 사용에 의한 정신 및 행동 장애'],
  ['F13','진정제 또는 수면제 사용에 의한 정신 및 행동 장애'],
  ['F14','코카인 사용에 의한 정신 및 행동 장애'],
  ['F15','카페인을 포함하는 기타 흥분제 사용에 의한 정신 및 행동 장애'],
  ['F16','환각제 사용에 의한 정신 및 행동 장애'],
  ['F17','담배흡연에 의한 정신 및 행동 장애'],
  ...
  ['F60','특정 인격장애'],
  ['F61','혼합형 및 기타 인격장애'],
  ['F62','뇌 손상과 질병이 원인이 아닌 지속적인 인격변화'],
  ['F63','습관 및 충동 장애'],
  ['F64','성주체성장애'],
  ['F65','성선호장애'],
  ...
  ['G13','달리 분류된 질환에서의 일차적으로 중추신경계통에 영향을 주는 계통성 위축'],
  ['G14','소아마비후증후군'],
  ['G20','파킨슨병'],
  ['G21','이차성 파킨슨증'],
  ...
  ['H01','눈꺼풀의 기타 염증'],
  ['H02','눈꺼풀의 기타 장애'],
  ['H03','달리 분류된 질환에서의 눈꺼풀의 장애'],
  ['H04','눈물계통의 장애'],
  ['H05','안와의 장애'],
  ['H06','달리 분류된 질환에서의 눈물계통 및 안와의 장애'],
  ['H10','결막염'],
  ['H11','결막의 기타 장애'],
  ...
  ['H57','눈 및 눈부속기의 기타 장애'],
  ['H58','달리 분류된 질환에서의 눈 및 눈부속기의 기타 장애'],
  ['H59','달리 분류되지 않은 눈 및 눈부속기의 처치후 장애'],
  ['H60','외이염'],
  ['H61','외이의 기타 장애'],
  ['H62','달리 분류된 질환에서의 외이의 장애'],
  ['H65','비화농성 중이염'],
  ['H66','화농성 및 상세불명의 중이염'],
  ['H67','달리 분류된 질환에서의 중이염'],
  ['H68','귀인두관염 및 귀인두관 폐색'],
  ['H69','귀인두관의 기타 장애'],
  ['H70','유돌염 및 관련 병태'],
  ['H71','중이의 진주종'],
  ...
  ['H73','고막의 기타 장애'],
  ['H74','중이 및 유돌의 기타 장애'],
  ['H75','달리 분류된 질환에서의 중이 및 유돌의 기타 장애'],
  ['H80','귀경화증'],
  ['H81','전정기능의 장애'],
  ['H82','달리 분류된 질환에서의 현기증후군'],
  ['H83','내이의 기타 질환'],
  ['H90','전음성 및 감각신경성 청력소실'],
  ['H91','기타 청력소실'],
  ['H92','귀통증 및 귀의 삼출액'],
  ['H93','달리 분류되지 않은 귀의 기타 장애'],
  ['H94','달리 분류된 질환에서의 귀의 기타 장애'],
  ['H95','달리 분류되지 않은 귀 및 유돌의 처치후 장애'],
  ...
  ['K90','장흡수장애'],
  ['K91','달리 분류되지 않은 소화계통의 처치후 장애'],
  ['K92','소화계통의 기타 질환'],
  ['K93','달리 분류된 질환에서의 기타 소화기관장애'],
  ['L00','포도알균성 화상피부증후군'],
  ['L01','농가진'],
  ['L02','피부의 농양, 종기 및 큰종기'],
  ['L03','연조직염'],
  ['L04','급성 림프절염'],
  ['L05','모소낭'],
  ['L08','피부 및 피하조직의 기타 국소감염'],
  ['L10','천포창'],
  ['L11','기타 극세포분리장애'],
  ['L12','유사천포창'],
  ['L13','기타 수포성 장애'],
  ['L14','달리 분류된 질환에서의 수포성 장애'],
  ['L20','아토피성 피부염'],
  ['L21','지루피부염'],
  ['L22','기저귀[냅킨]피부염'],
  ['L23','알레르기성 접촉피부염'],
  ...
  ['L90','피부의 위축장애'],
  ['L91','피부의 비대성 장애'],
  ['L92','피부 및 피하조직의 육아종성 장애'],
  ['L93','홍반루푸스'],
  ['L94','기타 국소적 결합조직장애'],
  ['L95','달리 분류되지 않은 피부에 한정된 혈관염'],
  ['L97','달리 분류되지 않은 다리의 궤양'],
  ['L98','달리 분류되지 않은 피부 및 피하조직의 기타 장애'],
  ['L99','달리 분류된 질환에서의 피부 및 피하조직의 기타 장애'],
  ['M00','화농성 관절염'],
  ['M01','달리 분류된 감염성 및 기생충성 질환에서의 관절의 직접감염'],
  ['M02','반응성 관절병증'],
  ...
  ['M93','기타 골연골병증'],
  ['M94','연골의 기타 장애'],
  ['M95','근골격계통 및 결합조직의 기타 후천성 변형'],
  ['M96','달리 분류되지 않은 처치후 근골격장애'],
  ['M99','달리 분류되지 않은 생체역학적 병변'],
  ['N00','급성 신염증후군'],
  ['N01','급속 진행성 신염증후군'],
  ['N02','재발성 및 지속성 혈뇨'],
  ['N03','만성 신염증후군'],
  ...
  ['O34','골반기관의 알려진 또는 의심되는 이상에 대한 산모관리'],
  ...
  ['O98','달리 분류될 수 있지만 임신, 출산 및 산후기에 합병된 산모의 감염성 및 기생충성 질환'],
  ...
  ['P05','태아성장지연 및 태아영양실조'],
  ['P07','달리 분류되지 않은 단기임신 및 저체중출산에 관련된 장애'],
  ...
  ['P35','선천성 바이러스질환'],
  ['P36','신생아의 세균패혈증'],
  ['P37','기타 선천성 감염 및 기생충질환'],
  ['P38','경도의 출혈을 동반하거나 동반하지 않은 신생아배꼽염'],
  ...
  ['P56','용혈질환으로 인한 태아수종'],
  ['P57','핵황달'],
  ['P58','기타 과다용혈로 인한 신생아황달'],
  ['P59','기타 및 상세불명의 원인으로 인한 신생아황달'],
  ['P60','태아 및 신생아의 파종성 혈관내응고'],
  ['P61','기타 출생전후기 혈액학적 장애'],
  ['P70','태아 및 신생아에 특이한 일과성 탄수화물대사장애'],
  ['P71','일과성 신생아 칼슘 및 마그네슘 대사장애'],
  ['P72','기타 일과성 신생아내분비장애'],
  ['P74','기타 일과성 신생아 전해질 및 대사 장애'],
  ['P75','낭성 섬유증에서의 태변장폐색(E84.1†)'],
  ['P76','신생아의 기타 장폐쇄'],
  ...
  ['Q21','심장중격의 선천기형'],
  ['Q22','폐동맥판 및 삼첨판의 선천기형'],
  ['Q23','대동맥판 및 승모판의 선천기형'],
  ['Q24','심장의 기타 선천기형'],
  ['Q25','대동맥혈관의 선천기형'],
  ['Q26','대정맥혈관의 선천기형'],
  ['Q27','말초혈관계통의 기타 선천기형'],
  ...
  ['Q42','대장의 선천 결여, 폐쇄 및 협착'],
  ['Q43','장의 기타 선천기형'],
  ['Q44','담낭, 담관 및 간의 선천기형'],
  ['Q45','소화계통의 기타 선천기형'],
  ['Q50','난소, 난관 및 넓은인대의 선천기형'],
  ['Q51','자궁 및 자궁경부의 선천기형'],
  ['Q52','여성생식기의 기타 선천기형'],
  ['Q53','미하강고환'],
  ['Q54','요도하열'],
  ...
  ['Q84','외피의 기타 선천기형'],
  ['Q85','달리 분류되지 않은 모반증'],
  ['Q86','달리 분류되지 않은 알려진 외인으로 인한 선천기형증후군'],
  ['Q87','다발계통에 영향을 주는 기타 명시된 선천기형증후군'],
  ['Q89','달리 분류되지 않은 기타 선천기형'],
  ['Q90','다운증후군'],
  ['Q91','에드워즈증후군 및 파타우증후군'],
  ['Q92','달리 분류되지 않은 보통염색체의 기타 삼염색체증 및 부분삼염색체증'],
  ['Q93','달리 분류되지 않은 보통염색체의 단일염색체증 및 결손'],
  ['Q95','달리 분류되지 않은 균형된 재배열 및 구조표지자'],
  ['Q96','터너증후군'],
  ['Q97','달리 분류되지 않은 여성표현형의 기타 성염색체이상'],
  ['Q98','달리 분류되지 않은 남성표현형의 기타 성염색체이상'],
  ['Q99','달리 분류되지 않은 기타 염색체이상'],
  ['R00','심장박동의 이상'],
  ['R01','심잡음 및 기타 심음'],
  ['R02','달리 분류되지 않은 괴저'],
  ['R03','진단명 없는 혈압수치이상'],
  ...
  ['R14','고창 및 연관된 병태'],
  ['R15','대변실금'],
  ['R16','달리 분류되지 않은 간비대 및 비장비대'],
  ['R17','황달을 동반하거나 동반하지 않은 달리 분류되지 않은 과빌리루빈혈증'],
  ['R18','복수'],
  ['R19','소화계통 및 복부의 기타 증상 및 징후'],
  ['R20','피부감각의 장애'],
  ['R21','발진 및 기타 비특이성 피부발진'],
  ['R22','피부 및 피하조직의 국소적 부기, 종괴 및 덩이'],
  ['R23','기타 피부변화'],
  ['R25','이상불수의운동'],
  ['R26','보행과 이동의 이상'],
  ['R27','기타 협조결여'],
  ...
  ['R47','달리 분류되지 않은 언어장애'],
  ['R48','달리 분류되지 않은 난독증 및 기타 상징적 기능이상'],
  ['R49','음성장애'],
  ['R50','기타 및 원인미상의 열'],
  ['R51','두통'],
  ['R52','달리 분류되지 않은 통증'],
  ...
  ['R86','남성생식기관 검사물의 이상소견'],
  ['R87','여성생식기관 검사물의 이상소견'],
  ['R89','기타 기관, 계통 및 조직 검사물의 이상소견'],
  ['R90','중추신경계통의 진단영상검사상 이상소견'],
  ['R91','폐의 진단영상검사상 이상소견'],
  ['R92','유방의 진단영상검사상 이상소견'],
  ['R93','기타 신체구조의 진단영상검사상 이상소견'],
  ['R94','기능검사의 이상결과'],
  ['R95','영아돌연사증후군'],
  ['R96','원인 미상의 기타 돌연사'],
  ['R98','지켜본 사람이 없었던 사망'],
  ...
  ['U08','코로나19의 개인력'],
  ['U09','코로나19 이후 병태'],
  ['U10','코로나19와 관련된 다발계통염증성증후군'],
  ['U11','코로나19에 대한 예방접종의 필요'],
  ['U12','치료용으로 사용시 유해작용을 나타내는 코로나19 백신'],
  ['U50','육음병증(六淫病證)'],
  ['U52','태양병증(太陽病證)'],
  ['U53','양명병증(陽明病證)'],
  ['U54','소양병증(少陽病證)'],
  ...
  ['V04','대형화물차 또는 버스와 충돌로 다친 보행자'],
  ['V05','열차 또는 철도차량과 충돌로 다친 보행자'],
  ['V06','기타 무동력 차량과 충돌로 다친 보행자'],
  ['V09','기타 및 상세불명의 운수사고에서 다친 보행자'],
  ['V10','보행자 또는 동물과 충돌로 다친 자전거 탑승자'],
  ['V11','다른 자전거와 충돌로 다친 자전거 탑승자'],
  ['V12','이륜 또는 삼륜자동차와 충돌로 다친 자전거 탑승자'],
  ['V13','승용차, 픽업트럭 또는 밴과 충돌로 다친 자전거 탑승자'],
  ['V14','대형화물차 또는 버스와 충돌로 다친 자전거 탑승자'],
  ['V15','열차 또는 철도차량과 충돌로 다친 자전거 탑승자'],
  ['V16','기타 무동력차량과 충돌로 다친 자전거 탑승자'],
  ['V17','고정 또는 정지된 물체와의 충돌로 다친 자전거 탑승자'],
  ['V18','비충돌 운수사고에서 다친 자전거 탑승자'],
  ['V19','기타 및 상세불명의 운수사고에서 다친 자전거 탑승자'],
  ...
  ['V24','대형화물차 또는 버스와 충돌로 다친 모터사이클 탑승자'],
  ['V25','열차 또는 철도차량과 충돌로 다친 모터사이클 탑승자'],
  ['V26','기타 무동력차량과 충돌로 다친 모터사이클 탑승자'],
  ['V27','고정 또는 정지된 물체와의 충돌로 다친 모터사이클 탑승자'],
  ['V28','비충돌 운수사고에서 다친 모터사이클 탑승자'],
  ['V29','기타 및 상세불명의 운수사고에서 다친 모터사이클 탑승자'],
  ...
  ['V34','대형화물차 또는 버스와 충돌로 다친 삼륜자동차 탑승자'],
  ['V35','열차 또는 철도차량과 충돌로 다친 삼륜자동차 탑승자'],
  ['V36','기타 무동력차량과 충돌로 다친 삼륜자동차 탑승자'],
  ['V37','고정 또는 정지된 물체와의 충돌로 다친 삼륜자동차 탑승자'],
  ['V38','비충돌 운수사고에서 다친 삼륜자동차 탑승자'],
  ['V39','기타 및 상세불명의 운수사고에서 다친 삼륜자동차 탑승자'],
  ['V40','보행자 또는 동물과의 충돌로 다친 승용차 탑승자'],
  ['V41','자전거와 충돌로 다친 승용차 탑승자'],
  ['V42','이륜 또는 삼륜자동차와의 충돌로 다친 승용차 탑승자'],
  ['V43','승용차, 픽업트럭 또는 밴과 충돌로 다친 승용차 탑승자'],
  ['V44','대형화물차 또는 버스와의 충돌로 다친 승용차 탑승자'],
  ['V45','열차 또는 철도차량과의 충돌로 다친 승용차 탑승자'],
  ['V46','기타 무동력 차량과의 충돌로 다친 승용차 탑승자'],
  ['V47','고정 또는 정지된 물체와의 충돌로 다친 승용차 탑승자'],
  ['V48','비충돌 운수사고에서 다친 승용차 탑승자'],
  ['V49','기타 및 상세불명의 운수사고에서 다친 승용차 탑승자'],
  ...
  ['V54','대형화물차 또는 버스와의 충돌로 다친 픽업트럭 또는 밴 탑승자'],
  ['V55','열차 또는 철도차량과의 충돌로 다친 픽업트럭 또는 밴 탑승자'],
  ['V56','기타 무동력차량과의 충돌로 다친 픽업트럭 또는 밴 탑승자'],
  ['V57','고정 또는 정지된 물체와의 충돌로 다친 픽업트럭 또는 밴 탑승자'],
  ['V58','비충돌 운수사고에서 다친 픽업트럭 또는 밴 탑승자'],
  ['V59','기타 및 상세불명의 운수사고에서 다친 픽업트럭 또는 밴 탑승자'],
  ...
  ['V64','대형화물차 또는 버스와의 충돌로 다친 대형화물차 탑승자'],
  ['V65','열차 또는 철도차량과의 충돌로 다친 대형화물차 탑승자'],
  ['V66','기타 무동력차량과의 충돌로 다친 대형화물차 탑승자'],
  ['V67','고정 또는 정지된 물체와의 충돌로 다친 대형화물차 탑승자'],
  ['V68','비충돌 운수사고에서 다친 대형화물차 탑승자'],
  ['V69','기타 및 상세불명의 운수사고에서 다친 대형화물차 탑승자'],
  ['V70','보행자 또는 동물과의 충돌로 다친 버스 탑승자'],
  ['V71','자전거와 충돌로 다친 버스 탑승자'],
  ['V72','이륜 또는 삼륜자동차와 충돌로 다친 버스 탑승자'],
  ['V73','승용차, 픽업트럭 또는 밴과 충돌로 다친 버스 탑승자'],
  ['V74','대형화물차 또는 버스와 충돌로 다친 버스 탑승자'],
  ['V75','열차 또는 철도차량과의 충돌로 다친 버스 탑승자'],
  ['V76','기타 무동력차량과 충돌로 다친 버스 탑승자'],
  ['V77','고정 또는 정지된 물체와의 충돌로 다친 버스 탑승자'],
  ['V78','비충돌 운수사고에서 다친 버스 탑승자'],
  ['V79','기타 및 상세불명의 운수사고에서 다친 버스 탑승자'],
  ...
  ['V87','피해자의 교통수단을 알지 못하는 명시된 형태의 교통사고'],
  ['V88','피해자의 교통수단을 알지 못하는 명시된 형태의 비교통사고'],
  ['V89','상세불명 차량형태의 자동차 또는 무동력차 사고'],
  ['V90','익사 및 익수의 원인이 된 선박사고'],
  ['V91','기타 손상의 원인이 된 선박사고'],
  ['V92','선박사고가 없는 수상운수 관련 익사 및 익수'],
  ['V93','익사 및 익수의 원인이 아닌 선박사고가 없는 선상사고'],
  ['V94','기타 및 상세불명의 수상운수사고'],
  ['V95','탑승자가 다친 동력항공기 사고'],
  ...
  ['W17','기타 한 면에서 다른 면으로 넘어짐'],
  ['W18','동일면상에서의 기타 낙상'],
  ['W19','상세불명의 낙상'],
  ['W20','던져진, 쏘아진 또는 떨어지는 물체에 의한 타격'],
  ['W21','스포츠장비에 의한 타격'],
  ['W22','다른 물체에 의한 타격'],
  ['W23','물체 속이나 사이에 붙잡힘, 으깨짐, 뭉개짐 또는 끼임'],
  ['W24','달리 분류되지 않은 인양 및 변속장치와의 접촉'],
  ['W25','날카로운 유리와 접촉'],
  ['W26','기타 날카로운 물체와 접촉'],
  ['W27','무동력 손공구와 접촉'],
  ['W28','동력잔디깎기와의 접촉'],
  ['W29','기타 동력손공구 및 가정용 기계류와의 접촉'],
  ['W30','농업용기계와 접촉'],
  ['W31','기타 및 상세불명의 기계류와 접촉'],
  ['W32','권총 발사'],
  ['W33','라이플, 엽총 및 기타 큰 화기발사'],
  ['W34','기타 및 상세불명의 화기발사'],
  ['W35','보일러 폭발 및 파열'],
  ['W36','가스통 폭발 및 파열'],
  ['W37','압축 타이어, 파이프 또는 호스의 폭발 및 파열'],
  ['W38','기타 명시된 가압장치의 폭발 및 파열'],
  ['W39','불꽃발사'],
  ['W40','기타 물질의 폭발'],
  ['W41','고압 분사에 노출'],
  ['W42','소음에 노출'],
  ['W43','진동에 노출'],
  ['W44','눈 또는 인체의 개구부를 통하여 들어온 이물'],
  ['W45','피부를 통하여 들어온 이물 또는 물체'],
  ['W46','피하 주사바늘과 접촉'],
  ['W49','기타 및 상세불명의 무생물성 기계적 힘에 노출'],
  ['W50','타인에 의해 맞음, 부딪힘, 차임, 비틀림, 물림 또는 찰과'],
  ['W51','타인과 부딪힘 또는 받힘'],
  ['W52','군중 또는 사람의 쇄도에 의한 으깨짐, 밀림 또는 밟힘'],
  ['W53','쥐에 물림'],
  ['W54','개에 물림 또는 부딪힘'],
  ['W55','기타 포유동물에 의한 물림 또는 부딪힘'],
  ['W56','바다 동물과 접촉'],
  ['W57','무독액성 곤충 및 기타 무독액성 절지동물에 물림 또는 쏘임'],
  ['W58','악어에 물림 또는 부딪힘'],
  ['W59','기타 파충류에 물림 또는 으깨짐'],
  ['W60','식물의 가시 및 날카로운 잎사귀와 접촉'],
  ['W64','기타 및 상세불명의 생물성 기계적 힘에 노출'],
  ...
  ['W78','위 내용물의 흡입'],
  ['W79','기도폐색의 원인이 된 음식물의 흡입 및 섭취'],
  ['W80','기도 폐색의 원인이 된 기타 물건의 흡입 및 섭취'],
  ['W81','저산소환경에 갇힘'],
  ['W83','호흡의 기타 명시된 위협'],
  ['W84','호흡의 상세불명 위협'],
  ['W85','송전선에 노출'],
  ...
  ['W92','인공적 과잉열에 노출'],
  ['W93','인공적 과잉한랭에 노출'],
  ['W94','고압 및 저압 및 기압의 변화에 노출'],
  ['W99','기타 및 상세불명의 인공환경 요인에 노출'],
  ['X00','건물 또는 구조물 안에서 통제되지 않은 불에 노출'],
  ['X01','건물 또는 구조물이 아닌 곳에서 통제되지 않은 불에 노출'],
  ['X02','건물 또는 구조물안에서 통제된 불에 노출'],
  ['X03','건물 또는 구조물이 아닌 곳에서 통제된 불에 노출'],
  ['X04','높은 가연성 물질의 발화에 노출'],
  ['X05','잠옷의 발화 또는 녹아내림에 노출'],
  ['X06','기타 의류의 발화 또는 녹아내림에 노출'],
  ['X08','기타 명시된 연기, 불 및 불꽃에 노출'],
  ['X09','상세불명의 연기, 불 및 불꽃에의 노출'],
  ['X10','뜨거운 음료 식품, 지방 및 조리용 기름과 접촉'],
  ['X11','뜨거운 수돗물과 접촉'],
  ['X12','기타 뜨거운 용액과 접촉'],
  ['X13','증기 및 뜨거운 김과 접촉'],
  ['X14','뜨거운 공기 및 가스와 접촉'],
  ['X15','뜨거운 가정기기와 접촉'],
  ['X16','뜨거운 난방장치, 방열기 및 파이프와 접촉'],
  ['X17','뜨거운 엔진, 기계 및 공구와 접촉'],
  ['X18','기타 뜨거운 금속과 접촉'],
  ['X19','기타 및 상세불명의 열 및 뜨거운 물질과 접촉'],
  ['X20','독액성 뱀 및 도마뱀과 접촉'],
  ['X21','독거미와 접촉'],
  ['X22','전갈과 접촉'],
  ...
  ['X25','기타 독액성 절지동물과 접촉'],
  ['X26','독액성 해양동물 및 식물과 접촉'],
  ['X27','기타 명시된 독액성 동물과 접촉'],
  ['X28','기타 명시된 독액성 식물과 접촉'],
  ...
  ['X34','지진에 의한 피해'],
  ['X35','화산폭발에 의한 피해'],
  ['X36','눈사태, 산사태 및 기타 지각운동에 의한 피해'],
  ['X37','지각변동 폭풍의 피해'],
  ['X38','홍수의 피해'],
  ['X39','자연의 기타 및 상세불명 힘에 노출'],
  ['X40','비아편유사진통제, 해열제 및 항류마티스제에 의한 불의의 중독 및 노출'],
  ['X41','달리 분류되지 않은 항뇌전증제, 진정제-수면제, 항파킨슨제, 정신작용제에 의한 불의의 중독 및 노출'],
  ['X42','달리 분류되지 않은 마약 및 환각제에 의한 불의의 중독 및 노출'],
  ['X43','자율신경계통에 작용하는 기타 약물에 의한 불의의 중독 및 노출'],
  ['X44','기타 및 상세불명의 약물, 약제 및 생물학적 물질에 의한 불의의 중독 및 노출'],
  ['X45','알코올에 의한 불의의 중독 및 노출'],
  ['X46','유기용제 및 할로겐화 탄화수소 및 그 휘발성 물질에 의한 불의의 중독 및 노출'],
  ['X47','일산화탄소 및 기타 가스, 휘발성 물질에 의한 불의의 중독 및 노출'],
  ['X48','유해생물방제제(농약)에 의한 불의의 중독 및 노출'],
  ['X49','기타 및 상세불명의 화학물질 및 독성물질에 의한 불의의 중독 및 노출'],
  ['X50','과잉노력 및 격심한 또는 반복적 운동'],
  ['X51','여행 및 이동'],
  ['X52','무중력환경에서의 장기간 체류'],
  ['X53','식량부족'],
  ['X54','물부족'],
  ...
  ['X61','달리 분류되지 않은 항뇌전증제, 진정제-수면제, 항파킨슨제 및 정신작용제에 의한 자의의 중독 및 노출'],
  ['X62','달리 분류되지 않은 마약 및 환각제에 의한 자의의 중독 및 노출'],
  ['X63','자율신경계통에 작용하는 기타 약물에 의한 자의의 중독 및 노출'],
  ['X64','기타 및 상세불명의 약물, 약제 및 생물학적 물질에 의한 자의의 중독 및 노출'],
  ['X65','알코올에 의한 자의의 중독 및 노출'],
  ['X66','유기용제 및 할로겐화 탄화수소 및 그 휘발성 물질에 의한 자의의 중독 및 노출'],
  ['X67','일산화탄소 및 기타 가스, 휘발성 물질에 의한 자의의 중독 및 노출'],
  ['X68','유해생물방제제(농약)에 의한 자의의 중독 및 노출'],
  ['X69','기타 및 상세불명의 화학물질 및 독성물질에 의한 자의의 중독 및 노출'],
  ['X70','목맴, 교액 및 질식에 의한 의도적 자해'],
  ['X71','익사 및 익수에 의한 의도적 자해'],
  ['X72','권총발사에 의한 의도적 자해'],
  ['X73','라이플, 엽총 및 기타 큰 화기발사에 의한 의도적 자해'],
  ['X74','기타 및 상세불명의 화기발사에 의한 의도적 자해'],
  ['X75','폭발성 물질에 의한 의도적 자해'],
  ['X76','연기, 불 및 불꽃에 의한 의도적 자해'],
  ['X77','증기, 뜨거운 김 및 뜨거운 물체에 의한 의도적 자해'],
  ['X78','예리한 물체에 의한 의도적 자해'],
  ['X79','둔한 물체에 의한 의도적 자해'],
  ['X80','높은 곳에서 뛰어내림에 의한 의도적 자해'],
  ['X81','움직이는 물체 앞에 뛰어들거나 누움에 의한 의도적 자해'],
  ['X82','자동차의 충돌에 의한 의도적 자해'],
  ['X83','기타 명시된 수단에 의한 의도적 자해'],
  ['X84','상세불명의 수단에 의한 의도적 자해'],
  ['X85','약물, 약제 및 생물학적 물질에 의한 가해'],
  ['X86','부식성 물질에 의한 가해'],
  ['X87','유해생물방제제(농약)에 의한 가해'],
  ['X88','일산화탄소, 기타 가스 및 휘발성 물질에 의한 가해'],
  ['X89','기타 명시된 화학물질 및 독성물질에 의한 가해'],
  ['X90','상세불명의 화학물질 또는 독성물질에 의한 가해'],
  ['X91','목맴, 교액 및 질식에 의한 가해'],
  ['X92','익사 및 익수에 의한 가해'],
  ['X93','권총발사에 의한 가해'],
  ['X94','라이플, 엽총 및 기타 소화기발사에 의한 가해'],
  ['X95','기타 및 상세불명의 화기발사에 의한 가해'],
  ['X96','폭발성 물질에 의한 가해'],
  ['X97','연기, 불 및 불꽃에 의한 가해'],
  ['X98','증기, 뜨거운 김 및 뜨거운 물체에 의한 가해'],
  ['X99','예리한 물체에 의한 가해'],
  ['Y00','둔한 물체에 의한 가해'],
  ['Y01','높은 곳에서 떠밀음에 의한 가해'],
  ['Y02','피해자를 움직이는 물체앞에 밀거나 놓이게 하여 가해'],
  ['Y03','자동차충돌에 의한 가해'],
  ['Y04','완력에 의한 가해'],
  ['Y05','완력에 의한 성적 학대'],
  ['Y06','방치 및 유기'],
  ['Y07','기타 학대'],
  ['Y08','기타 명시된 수단에 의한 가해'],
  ['Y09','상세불명의 수단에 의한 가해'],
  ...
  ['Y15','의도 미확인의 알코올에 의한 중독 및 노출'],
  ['Y16','의도 미확인의 유기용제 및 할로젠화 탄화수소 및 그 휘발성 물질에 의한 중독 및 노출'],
  ['Y17','의도 미확인의 일산화탄소 및 기타 가스 및 휘발성 물질에 의한 중독 및 노출'],
  ...
  ['Y42','치료용으로 사용시 유해작용을 나타내는 달리 분류되지 않은 호르몬 및 그들의 합성 대치물 및 길항제'],
  ...
  ['Y54','치료용으로 사용시 유해작용을 나타내는 주로 수분균형과 무기물 및 요산 대사에 영향을 주는 제제'],
  ['Y55','치료용으로 사용시 유해작용을 나타내는 주로 평활근 및 골격근과 호흡계통에 작용하는 제제'],
  ['Y56','치료용으로 사용시 유해작용을 나타내는 주로 피부 및 점막에 영향을 주는 국소약제와 안과, 이비인후과 및 치과 약물'],
  ...
  ['Y89','기타 외인의 후유증'],
  ['Y90','혈액 알코올농도로 확인된 알코올 관여의 증거'],
  ['Y91','중독의 농도가 확인된 알코올 관여의 증거'],
  ['Y95','병원과 관련된 상태'],
  ['Y96','업무와 관련된 상태'],
  ['Y97','환경오염과 관련된 상태'],
  ['Y98','생활방식과 관련된 상태'],
  ...
  ['Z41','건강상태개선 이외의 목적으로 이루어진 처치를 위하여 보건서비스와 접하고 있는 사람'],
  ...
  ['Z46','기타 장치의 부착 및 조정를 위하여 보건서비스와 접하고 있는 사람'],
  ['Z47','기타 정형외과적 추적치료를 위하여 보건서비스와 접하고 있는 사람'],
  ['Z48','기타 외과적 추적치료를 위하여 보건서비스와 접하고 있는 사람'],
  ['Z49','투석을 포함한 치료를 위하여 보건서비스와 접하고 있는 사람'],
  ['Z50','재활처치를 포함한 치료를 위하여 보건서비스와 접하고 있는 사람'],
  ['Z51','기타 의학적 관리를 위하여 보건서비스와 접하고 있는 사람'],
  ['Z52','기관 및 조직의 기증자'],
  ...
  ['Z86','특정 기타 질환의 개인력'],
  ['Z87','기타 질환 및 병태의 개인력'],
  ['Z88','약물, 약제 및 생물학적 물질에 대한 알레르기의 개인력'],
  ['Z89','후천성 사지결여'],
  ['Z90','달리 분류되지 않은 기관의 후천성 결여'],
  ['Z91','달리 분류되지 않은 위험 요인의 개인력'],
  ['Z92','의학적 치료의 개인력'],
  ['Z93','인공개구상태'],
  ['Z94','이식된 기관 및 조직의 상태'],
  ...
  ['Z97','기타 장치의 존재'],
  ['Z98','기타 수술후 상태'],
  ['Z99','달리 분류되지 않은 기능성 기계 및 장치에 대한 의존']
  ];

  /* ── 이 코드가 무엇인가 ────────────────────────────────────────
     설계사가 진단서를 볼 때 <b>가장 먼저 갈려야 하는 것</b>입니다.
     X00(불에 노출)을 질병으로 착각하면 상담이 통째로 어긋납니다.        */
  function kind(c) {
    var a = c.charAt(0);
    if ('VWXY'.indexOf(a) >= 0) return '외인';   /* 사고가 <b>어떻게</b> 났는가 — 진단명이 아님 */
    if (a === 'R') return '증상';                /* 확진 전의 증상·징후 */
    if (a === 'Z') return '보건';                /* 검진·예방접종·가족력 — 병이 아님 */
    if (a === 'U') return '한의';                /* 한의 고유코드 등 */
    return '질병';
  }

  /* ── 어느 담보 갈래를 보는가 ────────────────────────────────────
     <b>담보를 정하는 것이 아닙니다.</b> 「이 갈래를 펴 보십시오」 까지입니다.
     실제 지급은 그 상품 약관의 분류표가 정합니다.                       */
  function area(c) {
    if (kind(c) !== '질병') return null;
    var a = c.charAt(0), n = parseInt(c.slice(1), 10);
    if (a === 'C') return '암';
    if (a === 'D') {
      if (n <= 9)  return '암';          /* 제자리암 — 통상 유사암으로 갈립니다 */
      if (n <= 36) return '수술·입원';   /* 양성 신생물 — 진단비가 아니라 수술비 자리 */
      if (n <= 48) return '암';          /* 행동양식불명(경계성) — 통상 유사암 */
      return '수술·입원';                /* 혈액·면역 */
    }
    if (a === 'I') return '뇌·심장';
    if (a === 'F' && n <= 3) return '치매·간병';
    if (a === 'G' && (n === 30 || n === 31)) return '치매·간병';
    return '수술·입원';
  }

  /* ── 가장 가까운 상담자료 ───────────────────────────────────────
     40종 가운데 <b>그 코드를 실제로 다루는</b> 자료로만 잇습니다.
     억지로 붙이지 않습니다 — 없으면 null 이고, 화면이 그렇게 적습니다.

     규칙은 <b>여기 한 곳에만</b> 둡니다. 질병 자료의 codes[] 와 여기가
     따로 놀면 쌍둥이가 되므로, 점검이 두 쪽을 견줍니다.                  */
  var MAP = [
    /* ── 암 ── */
    [/^C(1[89]|2[01])$/,          'colon'],        /* 결장·직장·항문 */
    [/^C(6[4-8])$/,               'uro'],          /* 콩팥·요관·방광 */
    [/^C71$|^C72$/,               'braintumor'],
    [/^C5[3-7]$/,                 'cervix'],       /* 자궁·난소 */
    [/^C50$/,                     'cancer_female'],
    [/^C73$/,                     'thyroid'],
    [/^C61$/,                     'prostate'],
    [/^C4[34]$/,                  'skin'],
    [/^C(8[0-9]|9[0-6])$/,        'cancer_blood'],
    [/^C(0[0-9]|1[0-7]|2[0-6]|3[0-9])$/, 'cancer_major'],  /* 위·간·췌·폐 등 */
    /* 제자리암 — 통상 <b>유사암</b>으로 갈립니다. 그 이야기가 있는 자료로 잇습니다 */
    [/^D0[03]$|^D09$/,            'cancer_major'],
    [/^D01$/,                     'colon'],
    [/^D0[24]$/,                  'skin'],
    [/^D05$/,                     'cancer_female'],
    [/^D0[67]$/,                  'cervix'],
    ...
    [/^D12$/,                     'colon'],
    [/^D2[24]$/,                  'skin'],
    [/^D25$|^D26$|^D27$|^D28$/,   'myoma'],
    [/^D3[23]$/,                  'braintumor'],
    [/^D34$/,                     'thyroid'],
    [/^D(3[5-9]|4[0-8])$/,        'cancer_major'],
    [/^D(5[0-9]|6[0-9]|7[0-7])$/, 'cancer_blood'], /* 빈혈·혈액 */
    /* ── 뇌·심장 ── */
    [/^I6[0-9]$|^G4[56]$/,        'stroke'],
    [/^I2[0-5]$/,                 'mi'],
    [/^I4[7-9]$/,                 'afib'],
    [/^I50$|^I51$|^I11$/,         'hf'],
    [/^I3[4-9]$|^I0[0-9]$/,       'valve'],        /* 판막 — 류마티스열 뒤끝 포함 */
    [/^I7[1-3]$|^I74$/,           'aorta'],
    [/^I8[0-3]$|^I26$/,           'varicose'],     /* 정맥 혈전과 그것이 폐로 간 것 */
    [/^I10$|^I70$|^E78$/,         'chronic'],      /* 고혈압·동맥경화·고지혈증 */
    /* ── 노년·간병 ── */
    [/^F0[0-3]$|^G3[01]$/,        'dementia'],
    [/^G2[0-3]$/,                 'parkinson'],
    [/^M8[0-5]$|^S([23]2|72|82)$/,'osteo'],        /* 골다공증과 그 골절 */
    [/^M48$/,                     'stenosis'],
    [/^H(8[0-3]|9[0-5])$|^R42$/,  'hearing'],
    /* ── 만성·기타 ── */
    [/^N1[0-9]$/,                 'kidney'],
    [/^N2[0-9]$/,                 'stone'],
    [/^N4[0-2]$/,                 'prostate'],
    [/^N8[0-5]$|^N9[0-8]$/,       'myoma'],
    [/^K7[0-7]$/,                 'liver'],
    [/^F10$/,                     'liver'],        /* 알코올 — 간 자료가 다룹니다 */
    [/^K8[0-7]$/,                 'gallstone'],
    [/^K5[01]$/,                  'ibd'],
    [/^K(5[5-9]|6[0-4])$/,        'colon'],        /* 게실·치핵 등 대장·항문 */
    [/^K2[0-9]$|^K3[0-1]$/,       'gerd'],
    [/^K3[5-9]$|^K4[0-6]$/,       'surgery'],      /* 충수·탈장 */
    [/^E1[0-4]$|^H3[5-6]$/,       'chronic'],      /* 당뇨와 당뇨망막병증 */
    [/^G6[0-4]$/,                 'chronic'],      /* 당뇨병성 신경병증 자리 */
    [/^E0[3-7]$/,                 'thyroid_fn'],
    [/^E79$|^M1[01]$/,            'gout'],
    [/^M0[5-9]$/,                 'ra'],
    [/^M1[2-9]$|^M2[0-5]$|^M7[5-9]$/, 'joint'],
    [/^M5[0-4]$/,                 'joint'],
    [/^J4[45]$/,                  'copd'],
    [/^J(0[0-9]|1[0-8]|2[0-2])$/, 'pneumonia'],
    [/^J3[0-9]$|^J9[0-4]$/,       'asthma'],       /* 코·부비동, 기흉 */
    [/^H2[5-9]$|^H40$|^H4[1-2]$/, 'cataract'],
    [/^B0[23]$/,                  'zoster'],
    [/^L(2[0-9]|3[0-9]|4[0-9]|5[0-9])$/, 'skin'],
    [/^G8[0-3]$|^G40$|^G41$/,     'stroke']        /* 마비·경련 — 뇌 쪽에서 다룹니다 */
  ];
  function dz(c) {
  ...
  var BY = {};
  ROWS.forEach(function (r) { BY[r[0]] = r[1]; });

  window.DZ_KCD = {
    src: SRC,
    rows: ROWS,
    name: function (c) { return BY[c] || null; },
    kind: kind,
    area: area,
    dz: dz,
    /* 코드로도 이름으로도 찾습니다. 띄어쓰기는 무시합니다 */
    find: function (q, max) {
      q = (q || '').trim();
      if (!q) return [];
      var up = q.toUpperCase().replace(/\s/g, ''), sq = q.replace(/\s/g, '');
      var out = [], i, r, hitCode, hitName;
      for (i = 0; i < ROWS.length && out.length < (max || 80); i++) {
        r = ROWS[i];
        hitCode = r[0].indexOf(up) === 0;
        hitName = r[1].replace(/\s/g, '').indexOf(sq) >= 0;
        if (hitCode || hitName) out.push(r);
      }
      return out;
    }
  };
