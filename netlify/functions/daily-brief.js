/* ════════════════════════════════════════════════════════════════
   하루 한 장 — 사람마다 한 장을 미리 완성해 두는 서버 일감.

   <b>지금은 돌지 않는다.</b> 사장님 지시로 netlify.toml 의 예약을 껐다 —
   「서버 밤에 안 만들게, 내가 들어가서 확인할 거고」. 대표 브리핑을
   여시면 앱이 <b>그 자리에서</b> 모아 세운다(schOpen → schLoad).

   그래도 <b>지우지 않고 남겨 둔다.</b> 되살리시려면 netlify.toml 에
   네 줄을 되돌리면 그만이고, 앱 쪽 읽는 자리(SCH.pre)도 그대로 살아
   있어 그날부터 다시 표 아홉 개를 안 부른다(7번). 지워 버리면 그때
   가서 이 계산을 <b>처음부터 다시</b> 짜야 한다. 아래 규칙들 —
   직책 범위·모름과 0·실명 안 담기 — 은 그대로 지킨 채 둔다.
