...
(async () => {
  ...
  const sms = await page.evaluate(() => {
    ...
    return {
      cards: cards.length,
      withSms: cards.filter(c => c.textContent.includes('문자')).length,
      bodies: Object.keys(TT_MAP).map(id => ({
        who: TT_MAP[id].d.customer_name, kind: TT_MAP[id].sms || '(없음)',
        tel: smsTel(TT_MAP[id].d.phone), body: smsBody(TT_MAP[id])
      }))
    };
  });
  console.log('카드', sms.cards, '· 문자 단추', sms.withSms);
  sms.bodies.forEach(b => {
    console.log(`\n  [${b.kind}] ${b.who} · ${b.tel}`);
    if (b.body) console.log('   ' + b.body.replace(/\n/g, '\n   '));
  });
  await page.locator('.tt-card').first().screenshot({ path: OUT + '/b-sms.png' });
