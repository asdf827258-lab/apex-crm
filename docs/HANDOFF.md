...
- **사장님이 하실 것** — claude.ai/code 에서 이 환경의 Network access 를 `Full` 로
  바꾸시거나, Custom 에 `*.go.kr` 을 넣어 주십시오. 그러면 다음 달부터 저절로 돕니다.
- 그때까지는 검색(WebSearch)은 되므로 사람이 확인해서 갱신합니다.
  2026년 8월분은 그렇게 손으로 채웠습니다.
