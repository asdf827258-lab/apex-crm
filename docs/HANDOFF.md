...
- 제약을 푸는 SQL: `migration_36_db_source.sql` — **Supabase 에서 한 번 실행해야 합니다.**
- 종류 목록은 `app_config` 의 **`db_sources`** 한 줄(쉼표로 이어 붙임)에 있습니다.
  `app_config` 쓰기 정책이 `is_owner() or is_admin()` 이라 **대표·운영자만 고칩니다.**
  팀원 화면에서는 편집칸이 아예 안 보이고, 함수를 직접 불러도 서버가 막습니다.
- 화면 위치
  - DB 통합 CRM → **DB 관리** : 종류 열 · 종류 거르개 · (대표만) 「DB 종류 목록」 편집칸
...
- 검사: `scripts/check-dbsource.js` (31개)
