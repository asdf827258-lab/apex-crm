...
> ## ⛔ 먼저 읽으세요 — 토스증권은 접었습니다 (#212, 2026-08)
>
> **이 문서의 아래쪽은 토스증권을 '시세 1순위' 로 전제하고 쓰였습니다. 지금은
> 아닙니다.** 설계 기록으로 남겨 두었을 뿐이니, 토스 관련 절차(⓪ 절, `toss-discover`,
> `toss-probe`, `providers.toss` 채우기)는 **따라 하지 마세요.**
>
> 키·주소·토큰까지 전부 통과했고 `tokenIssued: true` 도 실제로 확인했습니다.
> 막힌 건 **허용 IP** 하나입니다 — 등록한 IP 로는 토큰이 나왔는데 배포 한 번에
> IP 가 바뀌어 `403 IP address not allowed` 가 났습니다. 토스는 호출 IP 를 미리
> 등록하라 하고 Netlify 는 배포마다 IP 가 바뀌니 등록으로는 쫓아갈 수 없습니다.
>
> **지금의 실제 순서는 ① 한국투자증권(실시간·지수) → ② 공공데이터포털(전일 종가)** 입니다.
> `providers.toss.paths.quote` 가 비어 있어 `tossReady()` 가 false 를 돌려주고,
> 그래서 토스는 네트워크 호출 없이 건너뜁니다.
>
> 잃는 것도 거의 없습니다. KIS 는 IP 등록이 없고 지수까지 줍니다. 종목 옆
> **`토스 ↗` 딥링크는 키와 무관하게 계속 동작합니다.**
>
> 현재 상태 요약은 `docs/이어서_하기.md`, 키 넣는 절차는 `docs/HANDOFF_INVEST.md`.

---

## 0. 토스는 어디에 어떻게 붙는가

> ⚠️ 이 절은 **접기 전** 기준으로 쓰인 기록입니다. 위 배너를 먼저 보세요.

토스 이름이 붙은 것이 **세 가지**인데 역할이 전부 다릅니다. 이걸 섞으면 헷갈립니다.

| 무엇 | 역할 | 이 CRM에서 |
|---|---|---|
| **토스증권 Open API** | OAuth 2.0 REST. 시세·계좌·주문 | ~~시세 1순위 제공자~~ → **접었습니다.** 설정은 남아 있지만 건너뜁니다 |
...
**키는 받았는데 문서의 경로를 모르겠다면** — `/api/market?kind=toss-discover` 를 한 번 부르세요.
발급받은 키로 **토스 도메인 안에서만** 후보 조합을 두들겨 보고, 토큰이 나오는 조합과 시세 경로를 찾아
`config/market.json` 에 그대로 붙여넣을 JSON(`suggestConfig`)을 만들어 줍니다.
문서에서 확인한 값이 있으면 `?base=…&token_path=…&quote_path=…` 로 지정해 좁힐 수도 있습니다.
(조회만 합니다. 주문 계열은 건드리지 않고, 탐색이 끝나면 설정을 원래대로 되돌립니다.)
...
### 토스 코드가 아직 없을 때 — 오늘 바로 켜는 길

승인을 기다리는 동안에도 화면을 비워 둘 필요가 없습니다.

| 길 | 걸리는 시간 | 성격 |
|---|---|---|
| **공공데이터포털** (금융위원회 주식·지수 시세정보) | **자동승인, 몇 분** · 계좌 불필요 | **전일 종가**(T+1). 장중 등락은 안 보이지만 평가금액·수익률·목표/손절 판단에는 충분 |
| 한국은행 ECOS | 즉시 발급 | 기준금리·환율·물가 — 경제동향 화면이 바로 켜집니다 |
| 한국투자증권 KIS | 30분 (계좌 개설 필요) | 실시간 |
| 토스증권 Open API | ⛔ 접음 (허용 IP 정책) | — |

**설정:** data.go.kr 에서 `금융위원회_주식시세정보`(15094808)와 `금융위원회_지수시세정보`(15094807)를
활용신청 → **일반 인증키(Decoding)** 를 `DATA_GO_KR_KEY` 환경변수에 넣고 재배포.
`/api/market?kind=krx-probe` 로 확인합니다.

> 종가 데이터로 표시된 값에는 화면에 **`종가 8/6`** 배지가 붙습니다.
> 실시간 제공자가 붙는 순간 배지는 사라지고 자동으로 그쪽 값으로 바뀝니다.

### 나머지 소스

| 데이터 | 제공자 | 성격 |
|---|---|---|
| 주식·ETF 현재가 | ① **한국투자증권** → ② **공공데이터포털** | ① 실시간 / ② 전일 종가 |
| 코스피·코스닥 지수 | **한국투자증권 KIS** → 없으면 **공공데이터포털** | 실시간 / 전일 종가 |
...
> 앞의 것이 없거나 실패하면 다음으로 자동으로 넘어갑니다. **여러 개 넣어두는 것을 권합니다** —
> 한쪽이 죽어도 화면이 비지 않습니다.
...
netlify/functions/market.js        ← 유일한 창구. 키는 전부 서버에만 있다.
   │  · 시세: KIS 1순위 → 실패하면 공공데이터포털(전일 종가) 자동 폴백
   │  · OAuth 토큰 캐시(동시 요청은 발급 1회로 묶어 호출제한 회피)
   │  · 종류별 TTL 캐시(시세 20초 · 지수 30초 · 지표/펀드 6시간 · 뉴스 15분)
   │  · 한 소스가 죽어도 나머지는 그대로 반환
   ▼
KIS / data.go.kr / ECOS / RSS
...
### ⓪ 토스증권 Open API — ⛔ 접었습니다. 따라 하지 마세요 (기록용)
...
| 증상 | 확인 |
|---|---|
| 전부 `—` 로 나온다 | `/api/market?kind=health` 의 `quoteProvider` 확인. `none` 이면 세 가지 다 미설정 |
| 값에 `종가 8/6` 배지가 붙는다 | 정상입니다 — 공공데이터포털(전일 종가)로 받고 있다는 뜻. 실시간을 원하면 토스나 KIS 를 넣으세요 |
| 공공데이터가 "등록되지 않은 서비스키" | 활용신청 승인 전이거나 Encoding 키를 넣은 경우. **Decoding 키**를 쓰세요 |
| 토스 경로를 모르겠다 | `/api/market?kind=toss-discover` 로 자동 탐색 |
...
| `scripts/check-market-api.js` | 시세 서버 자동 점검 — KIS 1순위·공공데이터 폴백·키 없을 때 동작 |
