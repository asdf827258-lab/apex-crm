...
(async () => {
  ...
  is(a.rows.length === 1 && a.rows[0] === '뇌혈관질환진단비=2,000만',
     '  느슨읽기 목록에서 빠진다 — ' + (a.rows.join(' · ') || '(없음)'));
  is(a.rider.length === 1 && a.rider[0] === '뇌혈관질환진단비=2,000만',
     '  <b>상품별 가입담보상세</b>에서도 빠진다 — ' + (a.rider.join(' · ') || '(없음)'));
