...
(async () => {
  ...
  /* ── 2026-09-22 · <b>왼쪽 목록에서는 빠졌다</b> ────────────────────
     사장님 말씀 「TFA업무관리에서 30일 고객관리는 … <b>기존 양식은
     지워버려</b>」. 「어떻게 이루었나」 는 홈 🌅 아침 미션 안의
     <b>📈 이번 달 어떻게 하셨나</b> 로 갔다.
     ★ <b>지운 것과 가른다</b> — 목록에는 없어야 하고, 불러서 열면
       그 판은 그대로 서야 한다(표에 남겨 둔 까닭이다 · 5번).
     ★ 30일 주기 <b>셈 자체</b>는 그대로다. 홈·달력·아침 미션이 모두
       그것으로 오늘 누구를 챙길지 정한다 — 아래 [1]~[11] 이 그것을 본다. */
  ok(r.cats.indexOf('30일 고객관리') < 0,
     'TFA 왼쪽에서 <b>30일 고객관리가 빠졌다</b> — 평가는 홈 아침 미션으로 갔다');
  ok(r.tbl === 1, '<b>지운 것이 아니다</b> — 불러서 열면 팀원별 표가 그대로 그려진다');
