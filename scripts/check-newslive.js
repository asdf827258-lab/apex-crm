/* 고객에게 전할 뉴스 — 다섯 칸으로 나뉘고, 정책·상품 뉴스와 이어지는가.
   ...
   여기서는 그것을 <b>고객에게 보낼 수 있는 모양</b>으로 바꾼 자리를 본다.
     ...
     1. 메뉴에 있고 열리는가
     2. 서버를 부를 때 늘린 네 칸(보험·경제·부동산·생활정책)을 다 부르는가
     3. 기사가 낱말대로 다섯 칸에 제대로 들어가는가
...
const srv = http.createServer((rq, rs) => {
  ...
  if (p === '/api/market') {
    ...
    return j({ ok: true, news: NEWS, keywords: [], cats: ['경제', '보험', '부동산', '생활정책'] });
  }
...
(async () => {
  ...
  is(g.cats.join(',') === 'ins,realty,tax,help,econ', '  칸이 다섯이다 — ' + g.cats.join(' · '));
  ...
  is(g.got === 8 && g.n === 6 && g.drop === 2,
     '  8건 받아 6건 남기고 2건 버렸다 — 받음 ' + g.got + ' · 남김 ' + g.n + ' · 버림 ' + g.drop);
