...
(async () => {
  ...
  /* 「30일 고객관리」 는 「오늘 터치할 사람」 바로 뒤에 온다.
     오늘 붙을 사람을 보고 나서, 한 달 안에 못 돈 사람을 본다. */
  /* 「대표 브리핑」 이 팀 관리 맨 끝에 들어왔다. 아침 7시에 서버가 만들어
     두는 한 장인데 왼쪽 큰 메뉴에만 서 있어서, 아침에 여는 이 화면에서는
     못 찾으셨다 — 「TFA 업무관리에서 대표관리가 현재 없고」. */
  /* 「교육매니저 할 일」 은 걷어냈습니다 — 사업단에서 안 쓰는 칸이었습니다.
     교육매니저 <b>역할</b>은 권한 관리에 그대로 있습니다. */
  /* 「DB · 업적관리」 가 교육매니저 할 일 바로 뒤(팀 관리 묶음 안)에 들어왔다.
  ...
  /* ── 2026-09-22 · <b>열 → 여덟</b> ─────────────────────────────────
     사장님 말씀 「내 업적 * 월간보고 우선 메뉴 없애기」 ·
     「TFA업무관리에서 30일 고객관리는 … 기존 양식은 지워버려」.
     목표는 📊 DB · 업적관리로 갔고(같은 줄 monthly_perf 를 쓴다),
     30일 고객관리가 하던 「어떻게 이루었나」 는 홈 아침 미션 안의
     <b>📈 이번 달 어떻게 하셨나</b> 로 갔다. */
  ok(cats.length === 8, '왼쪽에 카테고리 여덟 개가 선다 (' + cats.length + ')');
  ok(cats.join('|') === '피드백|본인 역량 체크|해야 할 일|고객 체크|리더 할 일|DB · 업적관리|팀원 관리|대표 브리핑',
    '순서가 요청대로다 — ' + cats.join(' · '));
  ok(cats.indexOf('내 업적 · 월간보고') < 0,
    'TFA 왼쪽에서 <b>내 업적 · 월간보고가 빠졌다</b> — 목표는 DB · 업적관리로 갔다');
  ok(cats.indexOf('30일 고객관리') < 0,
    'TFA 왼쪽에서 <b>30일 고객관리가 빠졌다</b> — 평가는 홈 아침 미션으로 갔다');
  /* <b>낼 길이 없어지면 안 된다.</b> 월간보고를 안 냈을 때 「해야 할 일」이
     그 줄을 세워 그 칸으로 보내는지 본다 — 아침 창은 하루 한 번만 뜬다. */
  const rep = await page.evaluate(() => {
    try {
      PF.loaded = true; PF.mine = { submitted_at: null };
      const has = (arTodoList() || []).filter(x => x.cat === 'perf');
      PF.mine = { submitted_at: '2026-09-01T00:00:00Z' };
      const gone = (arTodoList() || []).filter(x => x.cat === 'perf');
      return { has: has.length, gone: gone.length, t: (has[0] || {}).t || '' };
    } catch (e) { return { err: String(e) }; }
  });
  ok(rep.has === 1, '안 냈으면 「해야 할 일」에 <b>낼 길</b>이 선다 — ' + (rep.t || rep.err || '(없음)'));
  ok(rep.gone === 0, '  <b>내고 나면 사라진다</b> — 끝난 일을 세워 두지 않는다 (8번)');
