...
(async () => {
  ...
  /* 「고객 연락 관리」 가 「오늘 터치할 사람」 다음에 들어왔다.
     터치할 사람은 아직 고객이 아닌 DB 이고, 그 다음 칸이 이미 고객이 된 사람이다.
     DB → 고객 순서로 이어서 본다. */
  ok(cats.length === 11, '왼쪽에 카테고리 열한 개가 선다 (' + cats.length + ')');
  ok(cats.join('|') === '피드백|스케줄 관리|본인 역량 체크|해야 할 일|내 업적 · 월간보고|내 코칭|본인 점검란|오늘 터치할 사람|고객 연락 관리|리더 할 일|팀원 관리',
    '순서가 요청대로다 — ' + cats.join(' · '));
