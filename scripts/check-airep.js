...
(async () => {
  ...
  /* 「교육매니저 할 일」 이 리더 할 일 바로 뒤(팀 관리 묶음 안)에 들어왔다.
     여태 「교육매니저」 는 역할 이름만 있고 무엇을 챙기는지가 앱에 없었다. */
  /* 「DB · 업적관리」 가 교육매니저 할 일 바로 뒤(팀 관리 묶음 안)에 들어왔다.
     ...
     그 다음에 사람을 본다. */
  ok(cats.length === 14, '왼쪽에 카테고리 열네 개가 선다 (' + cats.length + ')');
  ok(cats.join('|') === '피드백|스케줄 관리|본인 역량 체크|해야 할 일|내 업적 · 월간보고|내 코칭|본인 점검란|오늘 터치할 사람|30일 고객관리|리더 할 일|교육매니저 할 일|DB · 업적관리|팀원 관리|대표 브리핑',
    '순서가 요청대로다 — ' + cats.join(' · '));
