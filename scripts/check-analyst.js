...
console.log('\n⑨ 한국어 부분 문자열 오탐 — 실제로 겪은 것들');

/* ⚠️ 낱말을 그대로 찾다가 화면에 이렇게 떴다. 전부 실물이다. */
function only(t){ return A.topicsOf(t).map(i => (A.TOPICS.filter(x => x.id === i)[0] || {}).name); }

ok("'유가'증권시장 → 유가·원자재 아님 (한국 증시 기사에 매일 나온다)",
   only('[표] 유가증권시장 2026년 상반기 연결 영업이익 상·하위 20개사').indexOf('유가·원자재') < 0,
   only('[표] 유가증권시장 2026년 상반기 연결 영업이익'));
ok("공'적자'금 → 실적 아님",
   only('예보, 서울보증보험 지분 0.6% 매각…공적자금 190억원 회수').indexOf('실적') < 0);
ok("세탁세'제' → 정책·규제 아님",
   only('소비자원 캡슐 세탁세제 세척력 제각각').indexOf('정책·규제') < 0);
ok("'6억달러' → 환율 아님 (달러는 금액 단위로 더 자주 쓰인다)",
   only('크리스 혼의 TCI, 이탈리아 특급호텔 대출에 6억달러 배팅').indexOf('환율') < 0);
ok("'303억달러 증발' → 환율 아님",
   only('AI가 띄운 SMR주 급락…시총 303억달러 증발').indexOf('환율') < 0);
ok("'정부, AI 창업 대전' → 반도체·AI 아님 (시장 기사가 아니다)",
   only('공공 혁신 스타트업 잡아라…정부, AI 창업 아이디어 대전').indexOf('반도체·AI') < 0);

/* 고치면서 맞는 것까지 놓치면 안 된다 */
ok('진짜 환율 기사는 잡는다', only('원/달러 환율 10개월 만에 1,300원대로 하락').indexOf('환율') >= 0);
ok('진짜 유가 기사는 잡는다', only('국제유가 90달러대 굳어질까').indexOf('유가·원자재') >= 0);
ok('진짜 실적 기사는 잡는다', only('삼성전자 2분기 영업이익 10조 돌파').indexOf('실적') >= 0);
ok('진짜 반도체 기사는 잡는다', only('SK하이닉스 HBM4 양산 앞당긴다').indexOf('반도체·AI') >= 0);
ok('진짜 금리 기사는 잡는다', only('한국은행 기준금리 동결').indexOf('금리·통화') >= 0);
ok('주담대·코픽스는 부동산으로', only('코픽스 넉달 연속 상승… 주담대 금리 또 오를듯').indexOf('부동산·건설') >= 0);
