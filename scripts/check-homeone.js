...
(async () => {
  ...
  /* ── 기준선 · 왜 3.0 인가 ─────────────────────────────────────────
     ...
     ── 2026-09-21 · 3.0 → <b>3.8</b> ────────────────────────────────
     사장님 말씀대로 <b>🌅 아침 미션 다섯</b>이 홈에 들어왔다 — 알림 확인 ·
     다섯 분 뉴스와 멘트 · 전화 다섯 통 · 카톡 다섯 건 · SNS 캐러셀.
     <b>얻은 것</b> — 아침에 홈만 보고 다 한다. DB 통합 CRM 으로 건너가지
       않는다. 체크는 실행 체크판 한 통에 그대로 담긴다.
     <b>치른 것</b> — 제일 긴 미션(③ 전화)일 때 0.9화면. 그래서 <b>접을 수
       있게</b> 했고(머리에 「2/5 · ③ 전화 다섯 통」 이 적혀 접어 두셔도
       어디까지 왔는지 보인다), 다섯 마디 틀은 기본으로 접었다.
     <b>아직 남은 것</b> — 「오늘 챙길 것」 의 한 분 카드(1,232px)가 TA
       자리에서는 미션 ②③ 과 겹친다. 둘을 넘기는 일은 여러 점검이 그
       카드를 보고 있어 이 판에서 안 했다.
     칸 자체의 자는 <b>check-msfive</b> 가 따로 든다 — 미션 칸은 한 화면
     (844px) 이하. 이쪽이 이 기능의 진짜 자다. */
  const hA = await tall(A.p);
  is(hA > 0 && hA <= 844 * 3.8,
     '  홈 높이 <b>' + hA + 'px</b> = 화면 ' + (hA / 844).toFixed(2) + '개 (3.8개 이하 · 고치기 전 4.2개)');
