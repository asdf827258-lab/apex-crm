...
var DBS=[
  ...
  /* ★ <b>여기가 안 울리던 자리다.</b> 위 세 줄은 지역과 시 칸이 <b>둘 다</b>
     「순천시」라, 시 이름으로 찾아도 지역 글자가 그대로 걸려 통과했다.
     그래서 시를 붙여 주는 cityOf 가 <b>죽어 있어도</b> 이 점검이 초록이었다.
     지역에 <b>「순천」</b>만 적고 시 칸을 비운 줄을 하나 둔다 — 이 줄은
     cityOf 가 살아 있어야만 「순천시」로 찾힌다 (8번).
     이름과 담당자는 <b>다른 줄과 안 겹치게</b> 둔다 — 겹치면 이름·담당자
     확인이 이 줄 때문에 흐려진다. */
  D('d5','장보고','010-5555-1212','순천','','u9','일반','')
];
...
(async () => {
  ...
  is((await rows()).length === 5, '치기 전에는 <b>다섯 건 전부</b> 보인다');
  ...
  /* ── <b>「순천」이라 적힌 분을 「순천시」로 찾을 수 있는가</b> ──────
     적는 사람마다 「순천」·「순천시」·「전남 순천시」로 갈립니다. 어느 쪽으로
     쳐도 같은 분이 나와야 합니다. 그 일을 하는 것이 cityOf 인데, 한동안
     <b>바깥에서 부를 수가 없어 늘 빈 글자</b>였습니다 — 화면이 안 깨지니
     아무도 못 봤습니다. 이 줄이 그 자리를 지킵니다. */
  r = await find('순천시');
  is(r.indexOf('장보고') >= 0,
     '지역에 <b>「순천」</b>만 적힌 분이 <b>「순천시」</b>로도 찾힌다 — 시를 붙여 주는 곳(cityOf)이 살아 있다 ('
     + (r.join(', ') || '아무도 안 나옴') + ')');
  r = await find('순천');
  is(r.indexOf('장보고') >= 0, '<b>「순천」</b>으로도 그대로 찾힌다');
  r = await find('박서준');
  ...
  is(r.length === 0, '「-」 한 글자로 <b>아무것도 안 걸린다</b> — 담당자 없는 줄이 딸려 오지 않는다 (' +
     r.join(', ') + ')');
  r = await find('홍길동');
  /* 홍길동은 <b>고객이자 담당자</b>다 — 본인 한 줄과, 본인이 맡은 줄(홍길순·홍만득)이 나온다.
     임꺽정은 박서준이 맡았으니 안 나와야 한다. 여기가 「담당자로도 찾는다」를 지키는 자리다. */
  ...
  is(/전체/.test(note) && /5/.test(note), '지우면 <b>전체 5건</b>으로 돌아온다 — 「' + note + '」');
  is((await rows()).length === 5, '줄도 다섯 건 전부 돌아왔다');
