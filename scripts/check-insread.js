...
(async () => {
  ...
  /* 없는 계열은 알려 주되 <b>「확정」 이라고는 말하지 않는다.</b>
     우리가 아는 것은 「읽어 낸 특약 안에 없다」 는 것뿐이다 — 스캔본이거나
     글자가 일부만 뽑히면 있는데 못 읽은 것일 수 있다. AI 에게 「확정」 이라
     하면 AI 는 「없습니다」 라고 리포트에 쓰고 <b>고객이 그것을 읽는다</b>
     (CLAUDE.md 1번). 실제로 오늘 특약을 통째로 버리던 자리가 그랬다. */
  is(/✕ 중입자·양성자 — 읽어 낸 특약 안에는 없음/.test(gapTxt.brief),
     'AI 에게 없는 계열을 알려 준다 — <b>읽어 낸 범위 안에서</b> 라고 밝히면서');
  /* 낱말 「확정」 만 보고 잡으면 「계약에 없다고 <b>확정된 것이 아니다</b>」
     라는 부정문까지 걸린다 — 물러서는 문장을 알람이 물어뜯는 꼴이다.
     헛것을 잡는 점검은 안 잡는 점검보다 나쁘다 (CLAUDE.md 8번).
     그래서 <b>단정하는 꼴만</b> 골라 잡는다. */
  const HARD = /없는 것이 확정|없음이 확정|확정적으로 없|미보유 확정|계약에 없습니다|가입되어 있지 않습니다|보장되지 않습니다/;
  is(!HARD.test(gapTxt.brief) && !HARD.test(gapTxt.card) && !HARD.test(gapTxt.deck),
     '「없는 것이 <b>확정</b>」 이라고 단정하는 꼴이 없다 — 결론은 어긋나면 그 자리에서 무너진다');
  is(!/✕ [^\n]*— 없음$/m.test(gapTxt.brief),
     'AI 에게 <b>맨 「없음」</b> 을 넘기지 않는다 — 범위를 안 밝히면 AI 가 「없습니다」 로 쓴다');
  is(/계약에 없다고 확정된 것이 아니다/.test(gapTxt.brief),
     'AI 에게 <b>확정이 아니라고 못을 박는다</b> — 안 박으면 AI 가 채워 넣는다');
  is(/단정해 쓰지 말고/.test(gapTxt.brief) && /증권 원본으로 확인/.test(gapTxt.brief),
     'AI 에게 <b>어떻게 쓰라고</b>까지 일러 준다 — 「확인이 필요합니다」 로');
  is(/증권 원본으로 한 번만/.test(gapTxt.card),
     '화면도 <b>원본으로 맞춰 보시라</b>고 적는다');
