...
/* ⚠️ 낱말을 그대로 찾으면 한국어에서 크게 헛짚는다. 실제로 겪은 것들:
      '유가'증권시장   → 유가·원자재로 갔다 (한국 증시 기사에 매일 나온다)
      공'적자'금       → 실적으로 갔다
      세탁세'제'       → 정책·규제로 갔다
      6억'달러'        → 환율로 갔다 (달러는 금액 단위로 훨씬 자주 쓰인다)
   그래서 낱말이 아니라 패턴으로 찾는다. words 는 사람이 읽으라고 남긴다. */
const TOPICS = [
  { id: 'semi', name: '반도체·AI',
    words: ['반도체', 'HBM', '메모리', '파운드리', 'AI', '엔비디아'],
    re: /반도체|HBM|D램|디램|낸드|파운드리|웨이퍼|GPU|엔비디아|TSMC|AI\s?(반도체|칩|가속기|데이터센터|서버|투자|수요)|생성형\s?AI/ },

  { id: 'rate', name: '금리·통화',
    words: ['금리', '연준', 'FOMC', '국채', '채권'],
    re: /금리|연준|FOMC|한국은행|금통위|국고채|국채|채권|긴축|테이퍼|양적완화/ },

  /* '달러' 를 그대로 넣으면 '6억달러' 가 전부 걸린다. 환율을 말하는 꼴만 찾는다. */
  { id: 'fx', name: '환율',
    words: ['환율', '원/달러', '달러 강세'],
    re: /환율|원\/달러|원달러|달러화|달러[ ·]?(강세|약세|가치|인덱스|지수)|엔화|위안화|외환시장|고환율|원화\s?(강세|약세|가치)/ },

  /* '정부'·'세제' 는 너무 흔하다. 시장에 닿는 꼴만 찾는다. */
  { id: 'policy', name: '정책·규제',
    words: ['금융위', '금감원', '규제', '관세', '세제 개편'],
    re: /금융위|금감원|공정위|국세청|규제|법안|과세|세제\s?(개편|개선|혜택)|감세|증세|조세|관세|제재|정부\s?(정책|대책|지원|발표|규제)/ },

  /* '적자' 는 '공적자금' 에 걸린다. 실적을 말하는 꼴만. */
  { id: 'earn', name: '실적',
    words: ['실적', '영업이익', '매출', '어닝'],
    re: /실적|영업이익|영업손실|순이익|순손실|매출액|매출\s?(증가|감소|성장|부진)|어닝\s?(서프라이즈|쇼크)|컨센서스|가이던스|적자\s?(전환|폭|지속)|흑자\s?전환/ },

  { id: 'share', name: '배당·주주환원',
    words: ['배당', '자사주', '주주환원'],
    re: /배당|자사주|주주환원|소각|무상증자|유상증자|액면분할|주주총회|행동주의/ },

  { id: 'battery', name: '2차전지',
    words: ['배터리', '2차전지', '전기차'],
    re: /배터리|[2이]차전지|전기차|양극재|음극재|리튬|ESS|충전\s?인프라/ },

  { id: 'bio', name: '바이오·제약',
    words: ['바이오', '제약', '임상', 'FDA'],
    re: /바이오|제약|임상\s?[0-9１-９]?상?|FDA|신약|CDMO|위탁생산|허가\s?신청/ },

  { id: 'ship', name: '조선·방산',
    words: ['조선', '수주', '방산'],
    re: /조선(사|업|소|주)?|LNG선|선박\s?수주|방산|무기\s?수출|전투기|잠수함|K-?방산/ },

  /* '유가' 는 '유가증권' 에 걸린다. 뒤에 '증권' 이 오면 아니다. */
  { id: 'energy', name: '유가·원자재',
    words: ['유가', '원유', 'OPEC', '금값'],
    re: /유가(?!증권)|국제유가|원유|WTI|브렌트|OPEC|천연가스|금값|구리\s?가격|원자재\s?(가격|시장)|호르무즈/ },

  { id: 'geo', name: '지정학',
    words: ['전쟁', '중동', '미중', '관세'],
    re: /전쟁|휴전|중동|우크라|대만\s?(해협|긴장)|북한|트럼프|미중\s?(갈등|무역|분쟁)|지정학/ },

  { id: 'realty', name: '부동산·건설',
    words: ['부동산', '분양', '주담대', '가계부채'],
    re: /부동산|아파트|분양|재건축|재개발|전세|주담대|주택담보|가계부채|코픽스|건설사/ }
];
...
function topicsOf(text) {
  const s = String(text || '');
  const hit = [];
  TOPICS.forEach(t => { if (t.re.test(s)) hit.push(t.id); });
  return hit;
}
