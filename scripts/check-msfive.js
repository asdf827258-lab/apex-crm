/* ══════════════════════════════════════════════════════════════════
   ...
   ── 보는 것 ───────────────────────────────────────────────────────
     ...
     [9] <b>칸이 한 화면을 안 넘는다</b> · 펴 놓아도 홈이 4.2화면 이하.
         ── 기준선 · 왜 3.8 인가 ────────────────────────────────────
         ...
         ── 2026-09-23 · <b>3.8 → 4.2</b> ──────────────────────────
         사장님 말씀대로(「목업대로 가줘」) 홈 맨 위에 <b>토스 카드 스택</b>
         이 섰다 — 인사·파랑 히어로·「지금 할 것」 까지 <b>438px</b>.
         미션을 펴 놓은 제일 긴 짝에서 홈이 3,207px → <b>3,388px</b>.
         자를 올리기 <b>전에</b> 겹치는 말 201px 을 먼저 걷어냈다 —
         무엇을 걷어냈는지는 <b>check-homeone</b> 의 같은 날 쪽지에
         한 곳에만 적어 둔다 (5번 · 같은 말을 두 곳에 두지 않는다).
         <b>얻은 것</b> — 홈을 열면 오늘 할 일이 큰 글씨 한 장으로 먼저
         선다. <b>치른 것</b> — 0.4화면. 접는 것은 그대로다.
...
(async () => {
  ...
  console.log('\n[9] <b>칸이 한 화면을 안 넘는다</b> · 홈은 4.2화면 이하');
  const B = await open({});
  await openMs(B.p);
  ...
  is(worstHome > 0 && worstHome <= 844 * 4.2,
     '  그때 홈이 <b>' + worstHome + 'px</b> = 화면 ' + (worstHome / 844).toFixed(2) + '개 (4.2 이하)');
