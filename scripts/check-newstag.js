...
/* ══════════════════════════════════════════════════════════════════
   ...
   <b>목록이 통째로 베껴진 것</b>이라, <b>한 대괄호 안에</b> 같은 통장
   낱말이 <b>다섯 개 이상</b> 몰려 있을 때만 잡습니다.
...
htmls.forEach(f => {
  ...
  KEYS.forEach((k, i) => {
    ...
    arrs.forEach(a => {
      ...
      if (n >= 5) dup.push(path.basename(f) + ' · ' + NAMES[i] + ' 낱말 ' + n + '개');
    });
  });
});
is(dup.length === 0,
...
const SAMPLE = [
  ...
  ['고령화로 장기요양·연금·상속 문제 한꺼번에 온다', '간병비와 요양등급, 연금 수급 개시, 상속세가 함께 걸린다'],
